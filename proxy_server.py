    ...
    try:
        ...
        try:
            ...
            if "operation not permitted" in str(e).lower() or e.errno == 1:
                ...
                print("[DNS 绑定失败] [错误代码 3004] DNS 解析绑定 tun0 失败，网卡设备不存在，请检查 节点连接！", flush=True)
            return None
        ...
        if proxy_auth_enabled():
            ...
            if not check_credentials(username, password):
                client.sendall(
                    ...
                    b"Proxy-Authenticate: Basic realm=\"Aimili Proxy\"\r\n"
                    b"Content-Length: 0\r\n\r\n"
                )
                return
    ...
    try:
        ...
        if is_ipv6 and host in ("::", ""):
            ...
            try:
                ...
                diag = nodepool_utils.diagnose_local_obstructions(port, host="0.0.0.0")
            ...
            try:
                ...
                import nodepool_utils
                diag = nodepool_utils.diagnose_local_obstructions(port, host="127.0.0.1")
                diag_msg = diag[1] if diag else str(ex)
                print(f"[ERROR] Failed to start HTTP/SOCKS5 proxy on 127.0.0.1:{port}: {diag_msg}", flush=True)
                return
        else:
            import nodepool_utils
            diag = nodepool_utils.diagnose_local_obstructions(port, host=host)
