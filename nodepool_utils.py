...
def enrich_ip_info(nodes: list[dict[str, Any]]) -> None:
    ...
    for i in range(0, len(ips_to_query), chunk_size):
        ...
        request = urllib.request.Request(
            ...
            data=payload,
            headers={"Content-Type": "application/json", "User-Agent": "nodepool-manager/2.2"},
            method="POST",
        )
        try:
    ...
    if sys.platform.startswith("linux"):
        ...
        if ip_forward_path.exists():
            try:
                ...
                if val == "0":
                    return 3001, "[ERR_ROUTE_FORWARD_DISABLED] 系统未开启 IPv4 流量转发。原因: /proc/sys/net/ipv4/ip_forward 值为 0，会导致 网络隧道内的流量无法进行正常的网络转发。"
            except Exception:
                pass
        ...
        try:
            ...
            if res.returncode == 0 and res.stdout.strip() == "active":
                return 3007, "[ERR_FIREWALL_BLOCKING_FORWARD] 本机 Firewalld 防火墙正在运行。请确保您已将代理端口及 虚拟网卡(tun0)加入信任区域以避免流量被拦截。"
        except Exception:
            pass
        ...
        try:
            ...
            if res.returncode == 0:
                ...
                if has_forward_drop:
                    return 3007, "[ERR_FIREWALL_BLOCKING_FORWARD] 本机 iptables FORWARD 默认策略被设为 DROP。且未配置相应的转发规则，这通常会拦截 虚拟网卡的流量穿透。"
        except Exception:
            pass
