...
NodePool 是一个自部署的分布式 OpenVPN 出站代理管理平台。支持 **主控端 (Master) + N 个被控端 (Agent)** 架构，自动从 VPNGate 等公开源拉取免费 OpenVPN 节点，经过住宅 IP 智能过滤、并发测速、故障自动漂移等处理后，在本机提供 HTTP / SOCKS5 代理出口。

> 适合需要海外出口 IP 轮换、多地区代理池、自动 Failover 的场景。
...
| 功能 | 说明 |
|------|------|
| **节点拉取与去重** | 从 VPNGate 接口抓取最新免费 OpenVPN 节点，拉取阶段即对 IP 智能去重归集 |
| **住宅 IP 智能过滤** | 集成 `ip-api.com`（首选）与 `ipapi.is`（备用）批量查询，自动保留住宅/移动 IP，过滤数据中心 IP，结果本地缓存 |
| **高性能并发测速** | 后台并发测试所有新节点的连通性、Ping 延迟与下载速度（1MB 测速），支持可调并发数与无上限测试 |
| **策略路由分流** | 基于 Policy Routing 实现流量分流，默认 `route-nopull` 防止 VPN 接管全局路由，保障 SSH 稳定 |
| **故障自动漂移** | 活动节点断线时自动感知，智能递归重试备用节点（单次最多 3 个），本地无可用节点时自动重新拉取 |
| **Web 管理后台** | 深色毛玻璃主题 UI，含仪表盘、节点列表、入站规则、代理设置、系统日志、自诊断等模块 |
| **系统自诊断** | 一键检测代理端口、OpenVPN 进程、虚拟网卡、DNS 状态，快速排查问题 |

### 🎛️ 主控端 (Master)

| 功能 | 说明 |
|------|------|
| **节点聚合** | 汇聚所有被控上传的节点，SHA256 指纹去重 |
| **L1 + L2 测活** | L1 TCP 连接测试 + L2 OpenVPN 真实握手测试（12s 超时），连续 3 次失败标记 dead |
| **按需下发** | 被控按地区查询活节点，主控智能匹配下发 |
| **Web Dashboard** | 轻量管理面板：节点池总览、地区分布、被控状态监控（🟢在线/🟡停滞/🔴离线/⏸️禁用） |
| **CLI 管理工具** | `master_admin.py` 命令行管理被控端的注册、启用、禁用、删除 |
| **安全防护** | 上传的 OpenVPN 配置自动扫描，禁用 `script-security`、`up`、`down` 等危险指令防 RCE |
...
运行后会提示选择：
- **1) 被控端 Agent** — 拉取节点 + 测速 + HTTP/SOCKS5 代理出口
- **2) 主控端 Master** — 聚合节点池 + L1/L2 测活 + Web Dashboard（默认端口 28080）

### 非交互式安装

适用于脚本 / CI 等自动化场景，通过参数直接指定角色：
...
> **安装目录**：默认 `/root/NodePool`
> 安装脚本会自动注册 systemd 守护进程并设为开机自启
...
```bash
systemctl status nodepool.service       # 查看状态
systemctl restart nodepool.service      # 重启服务
systemctl stop nodepool.service         # 停止服务（自动清理策略路由）
systemctl enable nodepool.service       # 开机自启
systemctl disable nodepool.service      # 取消自启
journalctl -u nodepool.service -f -n 100   # 实时日志
```

### 主控端 (Master)

```bash
systemctl status nodepool-master.service       # 查看状态
systemctl restart nodepool-master.service      # 重启服务
journalctl -u nodepool-master.service -f       # 实时日志

# CLI 管理工具
./master/master_admin.py status          # 节点池/被控总览
./master/master_admin.py list-agents     # 列出所有被控
./master/master_admin.py show-tokens     # 查看 enroll/admin token
./master/master_admin.py disable <id>    # 禁用被控
./master/master_admin.py enable <id>     # 启用被控
./master/master_admin.py delete <id>     # 删除被控
```
...
被控端连接节点后，会在本地启动 HTTP 与 SOCKS5 代理出口（默认端口 `10010`，以实际配置为准）：
...
配置文件：`master/master_data/master_config.json`（首启自动生成）

| 字段 | 默认值 | 说明 |
|------|--------|------|
| `listen_port` | `28080` | HTTP 监听端口 |
| `enroll_token` | 自动生成 | 被控注册口令 |
...
升级脚本会自动：拉取最新代码 → Python 语法安全检查 → 重启服务，失败时自动回滚。
...
**正常现象，可放心忽略。** 系统默认开启 `route-nopull` 防止 VPN 接管全局路由。VPN 服务端推送 DNS/默认路由时，客户端会拦截并打印此报错。只要后续出现 `Initialization Sequence Completed` 即表示 VPN 连接正常。

### Q: 拉取了大量节点但只有少数有延迟和网速？

- **固定地区模式**下，系统只测试目标国家的节点，其他国家显示"未检测"
- 切换到**自动配置 (Auto)** 模式会并发测试全部节点
- 随时可通过右上角**「一键测速」**强制测试当前列表所有节点

### Q: 提示"TUN 设备不可用"？

```bash
ls -l /dev/net/tun      # 检查 tun 设备
modprobe tun             # 手动加载模块
```

> 轻量级容器（Docker / LXC）需在宿主机配置中开启 TUN 映射。

---

## 🔒 安全建议

1. **修改默认凭证** — 首次安装后务必登录 Web 后台修改用户名、密码与安全路径
2. **端口隔离** — 不要将代理端口（默认 `10010`）监听至 `0.0.0.0`，如有内网需求建议防火墙白名单
3. **反向代理** — 推荐使用 Nginx 在 Web 面板前架设反向代理并开启 HTTPS
